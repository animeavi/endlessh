...
static enum loglevel { log_none, log_info, log_debug } loglevel = log_none;

static void (*logmsg)(enum loglevel level, const char *, ...);

static void logstdio(enum loglevel level, const char *format, ...) {
  if (loglevel >= level) {
    int save = errno;
...
static void logsyslog(enum loglevel level, const char *format, ...) {
...
static struct client *client_new(int fd, long long send_next) {
  ...
  if (c) {
    ...
    if (r == -1) logmsg(log_debug, "errno = %d, %s", errno, strerror(errno));
...
static void client_destroy(struct client *client) {
  ...
  logmsg(log_info,
         ...
         client->ipaddr, client->port, client->fd, dt / 1000, dt % 1000,
         client->bytes_sent);
...
static void statistics_log_totals(struct client *clients) {
  ...
  logmsg(log_info, "TOTALS connects=%lld seconds=%lld.%03lld bytes=%lld",
         statistics.connects, milliseconds / 1000, milliseconds % 1000,
...
static void fifo_init(struct fifo *q) {
  q->head = q->tail = 0;
  q->length = 0;
}

static struct client *fifo_pop(struct fifo *q) {
  struct client *removed = q->head;
  q->head = q->head->next;
  removed->next = 0;
  if (!--q->length) q->tail = 0;
  return removed;
}

static void fifo_append(struct fifo *q, struct client *c) {
  if (!q->tail) {
    q->head = q->tail = c;
  } else {
    q->tail->next = c;
...
static int randline(char *line, int maxlen, unsigned long s[1]) {
  ...
  for (int i = 0; i < len - 2; i++) line[i] = 32 + rand16(s) % 95;
  line[len - 2] = 13;
  line[len - 1] = 10;
  if (memcmp(line, "SSH-", 4) == 0) line[0] = 'X';
  return len;
}
...
#define CONFIG_DEFAULT                                                      \
  {                                                                         \
    .port = DEFAULT_PORT, .delay = DEFAULT_DELAY,                           \
    .max_line_length = DEFAULT_MAX_LINE_LENGTH,                             \
    .max_clients = DEFAULT_MAX_CLIENTS, .bind_family = DEFAULT_BIND_FAMILY, \
  }

static void config_set_port(struct config *c, const char *s, int hardfail) {
  errno = 0;
  char *end;
...
static void config_set_delay(struct config *c, const char *s, int hardfail) {
  errno = 0;
  char *end;
  long tmp = strtol(s, &end, 10);
...
static void config_set_max_clients(struct config *c, const char *s,
                                   int hardfail) {
  errno = 0;
  char *end;
  long tmp = strtol(s, &end, 10);
...
static void config_set_max_line_length(struct config *c, const char *s,
                                       int hardfail) {
  errno = 0;
  char *end;
  long tmp = strtol(s, &end, 10);
...
static void config_set_bind_family(struct config *c, const char *s,
                                   int hardfail) {
  switch (*s) {
    case '4':
...
static enum config_key config_key_parse(const char *tok) {
  static const char *const table[] = {[KEY_PORT] = "Port",
                                      [KEY_DELAY] = "Delay",
  ...
  for (size_t i = 1; i < sizeof(table) / sizeof(*table); i++)
    if (!strcmp(tok, table[i])) return i;
  return KEY_INVALID;
}

static void config_load(struct config *c, const char *file, int hardfail) {
  long lineno = 0;
  FILE *f = fopen(file, "r");
  if (f) {
    char line[256];
...
static void config_log(const struct config *c) {
...
static void usage(FILE *f) {
  fprintf(f,
          "Usage: endlessh [-vVhs] [-46] [-d MS] [-f CONFIG] [-l LEN] "
          "[-m LIMIT] [-p PORT]\n");
  fprintf(f, "  -4        Bind to IPv4 only\n");
  fprintf(f, "  -6        Bind to IPv6 only\n");
  fprintf(f,
          "  -d INT    Message millisecond delay [" XSTR(DEFAULT_DELAY) "]\n");
  fprintf(f,
          "  -f        Set and load config file [" DEFAULT_CONFIG_FILE "]\n");
  fprintf(f, "  -h        Print this help message and exit\n");
  fprintf(f, "  -l INT    Maximum banner line length (3-255) [" XSTR(
                 DEFAULT_MAX_LINE_LENGTH) "]\n");
  fprintf(f, "  -m INT    Maximum number of clients [" XSTR(
                 DEFAULT_MAX_CLIENTS) "]\n");
...
static void print_version(void) { puts("Endlessh " XSTR(ENDLESSH_VERSION)); }

static int server_create(int port, int family) {
  int r, s, value;
...
static struct client *sendline(struct client *client, int max_line_length,
                               unsigned long *rng) {
  char line[256];
...
int main(int argc, char **argv) {
  ...
  if (pledge("inet stdio rpath unveil", 0) == -1) die();
#endif
  ...
  while ((option = getopt(argc, argv, "46d:f:hl:m:p:svV")) != -1) {
    switch (option) {
      ...
      case 's':
        ...
        break;
      case 'v':
        if (loglevel < log_debug) loglevel++;
        break;
      case 'V':
        print_version();
        exit(EXIT_SUCCESS);
      ...
      if (fd == -1) {
        ...
        switch (errno) {
          ...
          case ENFILE:
            ...
            logmsg(log_info, "MaxClients %d", fifo->length);
            break;
          case ECONNABORTED:
          case EINTR:
          case ENOBUFS:
        ...
        if (!client) {
          ...
          logmsg(log_info, "ACCEPT host=%s port=%d fd=%d n=%d/%d",
                 client->ipaddr, client->port, client->fd, fifo->length,
                 config.max_clients);
        }
      }
    }
